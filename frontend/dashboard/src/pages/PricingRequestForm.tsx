...
import ChatPricingRequest from '../components/ChatPricingRequest';
...
export default function PricingRequestForm() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {/* Mode Toggle: Form vs Chat */}
      <div className="flex gap-2 mb-6">
        <button
          onClick={() => setMode('form')}
          className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${mode === 'form' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
        >
          Form
        </button>
        <button
          onClick={() => setMode('chat')}
          className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${mode === 'chat' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
        >
          AI Chat Assistant
        </button>
      </div>

      {mode === 'chat' && <ChatPricingRequest />}
